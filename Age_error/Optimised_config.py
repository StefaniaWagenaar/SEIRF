...
#data_model = 'Global_Model_WD_Internal_Release_2016_v3' 
...
if data_model.startswith('Global_Model_WD_Internal_Release'):
    ...
    #model_name = "13112023_trench_hotspot_250Ma"
    #model_name = "14112023_TM_HS_250Ma_newfiles"
    #model_name = "15112023_TM_HS_250Ma_plots"
    #model_name = "16112023_TM_HS_250Ma_continental_polygons"
    #model_name = "16112023_TM_HS_250Ma_search_radius60"
    #model_name = "17112023_TM_HS_250Ma_400models"
    #model_name = "18112023_TM_HS_250Ma_5Ma_400models"
    #model_name = "21112023_TM_HS_250Ma_10Ma_200models_comments"
    #model_name = "23112023_TM_HS_boundsTM_1"
    #model_name = "lat_test"
    #model_name = "30112023_angle_test"
    #model_name = "01122023_TM_HS_PV"
    #model_name = "05-12-2023_TM_HS_PV_NR" #includes debug arrays to discover what rules produce what cost
    #Put TM bounds back to -30,30
    #model_name = "07122023_PV" #Uses only PV to optimize, but exports all info on NR, TM, HS as well
    #model_name = "11122023_HS" #Uses only HS to optimize, but exports all info on NR, TM, PV as well
    #max_iter had to be set to 500 to make the model converge
    #model_name = "13122023_NR"
    #model_name = "14122023_TM"
    #model_name = "15122023_TM" #new run because the first one did not go right
    #model_name = "19122023_TM_HS_PV_NR_newdebug" #Changing the exported debug array to include more info
    #model_name = "20122023_PV" #Using only PV to optimize, new debug array
    #model_name = "21122023_NR" #Using only NR to optimize, new debug array
    #model_name = "09012024_HS_onlymedian" #uses the cost function of just the median distance for the hotspots, without the std
    #model_name = "10012024_HS" #second attempt to make the model converge with more time, no max_iter
    #model_name = "11012024_HS_no_Louisville" #without Louisville
    #model_name = "12012024_HS_pacific" #only pacific hotspots
    #model_name = "15012024_HS_indoatlantic" #only indo-atlantic hotspots
    #model_name = "17012024_HS_Tristan" #only tristan hotspot
    #model_name = "18012024_HS_Hawaii"
    #model_name = "24012024_HS_10My" #New interpolation method
    #model_name = "26012024_TM_HS_PV_NR_og" #Trying to recreate the original model
    #model_name = "06022024_TM_PV_NR" #Removing the hotspots to study their influence
    #model_name = "12022024_TM_HS_NR" #Removing PV to study its influence
    #model_name = "14022024_HS_5my" #New interpolation method at 5 myr
    #model_name = "16022024_TM_5my" #Running the model with just TM with 400 models and 5 myr intervals
    #model_name = "22022024_HS_5my_take2"
    #model_name = "26022024_HS_5my_take3" #Turned off the weighting according to uncertainty, Changed interpolated file
    #model_name = "Age_error_test"
    #model_name = "Age_error_test2"
    #model_name = "Age_error_all_rules_500"
    #model_name = "Age_error_NR_500"
    #model_name = "Age_error_NR_2_500"
    #model_name = "Age_error_PV_3"
    #model_name = "Age_error_PV_4"
    #model_name = "Age_error_PV_1000_Ma"
    #model_name = "Age_error_PV_mean_1000_Ma"
    model_name = "Age_error_PV_mean_1000_Ma_4"
...
#Variables that define the age error algorithm
window = 10
iterations = 500
...
if data_model.startswith('Global_Model_WD_Internal_Release'):
    ...
    #isochron_file = 'Global_EarthByte_230-0Ma_GK07_AREPS_Isochrons.gpmlz'
    #isocob_file = 'Global_EarthByte_230-0Ma_GK07_AREPS_IsoCOB.gpml'
    ridge_file = data_model + '/StaticGeometries/AgeGridInput/Global_EarthByte_GeeK07_Ridges_2019_v3.gpml'
    isochron_file = data_model + '/StaticGeometries/AgeGridInput/Global_EarthByte_GeeK07_Isochrons_2019_v3.gpml'
    isocob_file = data_model + '/StaticGeometries/AgeGridInput/Global_EarthByte_GeeK07_IsoCOB_2019_v3.gpml'
else:
    #
...
#  1. Enable boolean (True or False) to export data per model,
...
#  5. Enable boolean (True or False) to include rule in cost function
#
...
def get_fracture_zone_params(age):
    ...
    return False, 1.0, cost_function, None, False
...
def get_net_rotation_params(age):
    ...
    def cost_function(PTLong1, PTLat1, PTangle1, SPLong, SPLat, SPangle, SPLong_NNR, SPLat_NNR, SPangle_NNR, nr_over_interval):
        ...
        return np.mean(np.abs(PTangle1)) 
    ...
    if (data_model == 'Global_1000-0_Model_2017' or
        ...
        nr_bounds = (0.08, 0.20)
        if age <= 80:
            return  True, 1.0, cost_function, nr_bounds,False  # Weight is always 1.0 for 0-80Ma
        else:
            return  True, 1.0, cost_function, nr_bounds,False  # 1.0 gives a *multiplicative* weight of 1.0
    else:
...
def get_trench_migration_params(age):
    ...
    if (data_model == 'Global_1000-0_Model_2017' or
        ...
        tm_bounds = [-30, 30]
        if age <= 80:
            return True, 1.0, cost_function, tm_bounds,False  # Weight is always 1.0 for 0-80Ma
        else:
            # NOTE: These are inverse weights (ie, the constraint costs are *multiplied* by "1.0 / weight").
            return True, 1.0, cost_function, tm_bounds,False  # 2.0 gives a *multiplicative* weight of 0.5
    else:
...
def get_hotspot_trail_params(age):
    ...
    if age <= 80:
        return True, 1.0, cost_function, None,False  # Weight is always 1.0 for 0-80Ma
    else:
        return False, 1.0, cost_function, None,False
...
def get_plate_velocity_params(age):
    ...
    if (data_model == 'Global_1000-0_Model_2017' or
        ...
        pv_bounds = [0, 60]
        if age <= 80:
            return True, 1.0, cost_function, pv_bounds,True  # Weight is always 1.0 for 0-80Ma
        else:
            return True, 1.0, cost_function, pv_bounds,True  # 2.0 gives a *multiplicative* weight of 0.5
    else:
        return False, 1.0, cost_function, None
...
#include_chains = ['Tristan', 'Reunion', 'St_Helena', 'Foundation', 'Cobb', 'Samoa', 'Tasmantid', 
#                  'Hawaii']
#include_chains = ['Louisville', 'Foundation', 'Cobb', 'Samoa', 'Tasmantid', 
#                  'Hawaii']
#include_chains = ['Tristan', 'Reunion', 'St_Helena']
#include_chains = ['Tristan']
#include_chains = ['Hawaii']
#include_chains = ['Reunion', 'Foundation', 'Cobb', 'Samoa', 'Tasmantid', 
                #  'Hawaii']
...
#interpolated_hotspots = 'interpolated_hotspot_chains_10Myr2.xlsx'
#interpolated_hotspots = 'interpolated_hotspot_chains_5Myr3.xlsx'
