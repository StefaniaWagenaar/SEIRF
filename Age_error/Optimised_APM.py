...
if __name__ == '__main__':
    ...
    try: 
        ...
        """
        This first bit of code creates the loop over the iterations and produces
        an age range randomly drawn from a window
        
        """
        
        samples = np.arange(0,window,1)
        
        steps = np.arange(end_age+window,start_age,window)
        ...
        for k in range(iterations):
            ...
            for j in range(len(steps)):
                sample = np.random.randint(window,size=1)
                age_range.append(int((steps[j] - (window/2)) + sample))
                
            for j in range(len(age_range)):
                if j == 0:
                    intervals.append(int(age_range[j]))
                else:
                    intervals.append(int(age_range[j]-age_range[j-1]))
            ...
            if use_parallel != MPI4PY or mpi_rank == 0:
                ...
                """
                The optimised rotation updater file is edited to consider the age range, rather than
                construct the age range from the interval and end age
            ...
            for i in range(0, len(age_range)):
                ...
                if use_parallel != MPI4PY or mpi_rank == 0:
                    ...
                    enable_fracture_zones, fracture_zone_weight, fracture_zone_cost_func, fracture_zone_bounds,cost_fz = get_fracture_zone_params(ref_rotation_start_age)
                    enable_net_rotation, net_rotation_weight, net_rotation_cost_func, net_rotation_bounds,cost_net_rotation = get_net_rotation_params(ref_rotation_start_age)
                    enable_trench_migration, trench_migration_weight, trench_migration_cost_func, trench_migration_bounds,cost_trench_migration = get_trench_migration_params(ref_rotation_start_age)
                    enable_hotspot_trails, hotspot_trails_weight, hotspot_trails_cost_func, hotspot_trails_bounds,cost_hotspot = get_hotspot_trail_params(ref_rotation_start_age)
                    enable_plate_velocity, plate_velocity_weight, plate_velocity_cost_func, plate_velocity_bounds,cost_plate_velocity = get_plate_velocity_params(ref_rotation_start_age)
                    ...
                    params = [current_search_radius, rotation_uncertainty, search_type, current_models, model_stop_condition, max_iter,
                            ...
                            include_chains, interpolated_hotspot_trails, tm_method,cost_fz,cost_net_rotation,cost_trench_migration,cost_hotspot,cost_plate_velocity]
                ...
                (x, opt_n, N, lb, ub,  model_stop_condition, max_iter,
                    ...
                    seed_lons, seed_lats, cost_array) = startingConditions[:36]
                ...
                def run_optimisation(x, opt_n, N, lb, ub, model_stop_condition, max_iter, interval, rotation_file, 
                                    ...
                                    reformArray, trail_data, use_trail_age_uncertainty, trail_age_uncertainty_ellipse, tm_method, cost_array):
                    ...
                    obj_f = ObjectiveFunction(
                            ...
                            use_trail_age_uncertainty, trail_age_uncertainty_ellipse, tm_method, cost_array)
                    
                    
                    
                    #debug_list.append(debug_data)
                    ...
                    if numevals > 2000:
                        print('Number of iterations over 2000: ', numevals, '_x = ', x)
                    #print(opt.get_numevals(), 'iterations performed')
                    #print(opt.get_initial_step(x), ' initial step size')
                    sys.stdout.flush()
                    if ref_rotation_start_age<81:
                        for i in range( len(obj_f.debug_data_array)):
                            #cost = obj_f.debug_data_array[i][0]+obj_f.debug_data_array[i][2]+obj_f.debug_data_array[i][6]+(obj_f.debug_data_array[i][9]*10) #*10 added to correct for missing scaling factor
                            cost = obj_f.debug_data_array[i][0]+obj_f.debug_data_array[i][2]+obj_f.debug_data_array[i][8]+(obj_f.debug_data_array[i][11]) 
                            if cost == minf:
                                costs_debug = obj_f.debug_data_array[i]
                    else:
                        for i in range( len(obj_f.debug_data_array)):
                            #cost = obj_f.debug_data_array[i][0]+obj_f.debug_data_array[i][2]+(obj_f.debug_data_array[i][6]*10) #*10 added to correct for missing scaling factor
                            cost = obj_f.debug_data_array[i][0]+obj_f.debug_data_array[i][2]+(obj_f.debug_data_array[i][8]) 
                            if cost == minf:
                                costs_debug = obj_f.debug_data_array[i]
                ...
                runopt = partial(run_optimisation, opt_n=opt_n, N=N, lb=lb, ub=ub, 
                                ...
                                trail_age_uncertainty_ellipse=trail_age_uncertainty_ellipse, tm_method=tm_method, cost_array=cost_array)
                ...
                if use_parallel != MPI4PY or mpi_rank == 0:
                    ...
                    output_file = pr.saveResultsToPickle(data_array, data_array_labels_short, ref_rotation_start_age, 
                                                        ref_rotation_end_age, current_search_radius, xopt, current_models, model_name, cost_array, step, window)
                    ...
                    if ref_rotation_start_age <= 80:
                        min_results.append(np.array(rmin[['Age', 'Ang', 'Lat', 'Lon', 'Minimum', 'Model', 'NR', 'NR_my', 'TM', 'TM_r', 'TM_a',
                         'TM_mvo', 'TM_mavo', 'TM_mvp', 'HS', 'HS_m', 'HS_sd', 'PV', 'PV_m']])[0])
                    else:
                        min_results.append(np.array(rmin[['Age', 'Ang', 'Lat', 'Lon', 'Minimum', 'Model', 'NR', 'NR_my', 'TM', 'TM_r', 'TM_a',
                         'TM_mvo', 'TM_mavo', 'TM_mvp', 'PV', 'PV_m']])[0])
            ...
            if use_parallel != MPI4PY or mpi_rank == 0:
                ...
                #Make a rotation model out of the optimised rotation files to find the euler poles for Laurentia and Africa 
                #And put them in a file
                datadir = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'data', '')
                rotation_model = pgp.RotationModel(datadir + optimised_rotation_updater.get_optimised_rotation_filename())
                
                
                for n, moment in enumerate(age_range):
                    #print(min_results[n])
                    if moment <= 80:
                        Age, Ang, Lat, Lon, Minimum, Model, NR, NR_my, TM, TM_r, TM_a, TM_mvo, TM_mavo, TM_mvp, HS, HS_m, HS_sd, PV, PV_m = min_results[n]
                    else:
                        Age, Ang, Lat, Lon, Minimum, Model, NR, NR_my, TM, TM_r, TM_a, TM_mvo, TM_mavo, TM_mvp, PV, PV_m = min_results[n]
                        HS = 0
                        HS_m = 0
                        HS_sd = 0
                    
                    rot_701 = rotation_model.get_rotation(moment,701)
                    rot_101 = rotation_model.get_rotation(moment,101)
                    
                    lat_701, lon_701, angle_701 = rot_701.get_lat_lon_euler_pole_and_angle_degrees()
                    lat_101, lon_101, angle_101 = rot_101.get_lat_lon_euler_pole_and_angle_degrees()
                    
                    dictionary.append({'iteration': k, 'age': moment,
                                       'lat701': lat_701, 'lon701': lon_701,'angle701': angle_701,
                                       'lat101': lat_101, 'lon101': lon_101, 'angle101': angle_101, 'NR': NR, 'NR_my': NR_my,
                                     'TM': TM,'TM_r': TM_r,'TM_a': TM_a, 'TM_mvo': TM_mvo, 
                                     'TM_mavo': TM_mavo,'TM_mvp': TM_mvp, 'HS': HS, 'HS_m': HS_m, 'HS_sd': HS_sd,
                                     'PV': PV,'PV_m': PV_m})
                    
                    temp_dataframe = pd.DataFrame(dictionary)
                    #print(temp_dataframe)
                    outputdir = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'dataframes', '')
                    output_csv_filename = model_name + '_dataframe.csv'
                    temp_dataframe.to_csv(outputdir + output_csv_filename, sep=';')
        ...
        #outputdir = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'model_output', '')
        #dataframe.to_csv(outputdir + 'Results_age_error_iterations: ' + str(iterations) + '_' + str(model_name) + '.csv', sep=';')
