...
class ModelSetup():
    ...
    def modelStartConditions(params, data, plot=True):
        ...
        (geographical_uncertainty,
            ...
            tm_method,cost_fz,cost_net_rotation,cost_trench_migration,cost_hotspot,cost_plate_velocity) = params[:44]
        ...
        cost_array = [cost_fz, cost_net_rotation,cost_trench_migration,cost_hotspot,cost_plate_velocity]
        ...
        if auto_calc_ref_pole == True:
            ...
            #if ref_rot_latitude == 0 or ref_rot_latitude ==-0:
                #ref_rot_latitude = -90
        ...
        if plot:
            ...
            plt.title("n = " + str(len(seed_lons)) + "Start seed distribution")
            ax = plt.axes(projection=ccrs.Robinson())
            ax.coastlines(linewidth=0.25)
            ax.add_feature(cfeature.LAND, zorder=1, color='bisque')
            ax.gridlines(draw_labels=True)
            ipmag.plot_vgp(ax, seed_lons, seed_lats)
            ipmag.plot_pole(ax, ref_rot_longitude, ref_rot_latitude, geographical_uncertainty, color='red')
            plt.show()
        ...
        startingConditions = [x, opt_n, N, lb, ub, model_stop_condition, max_iter,
                              ...
                              seed_lons, seed_lats, cost_array, ang_gaussian_array]
...
class ProcessResults():
    ...
    @staticmethod
    def saveResultsToPickle(data_array, data_array_labels_short, ref_rotation_start_age, ref_rotation_end_age, geographical_uncertainty, xopt, models, model_name, cost_array, step, window):
        ...
        output_file = "model_output/" + model_name + "_" + str(int(step)) + "-" + str(int(step-window)) + "Ma_" \
         + str(models) + "models" + constraints + "_" + str(geographical_uncertainty) + ".pkl"
    ...
    def sortAndPlot(output_file, ref_rotation_start_age, ref_rotation_end_age, rotation_age_of_interest_age, xopt, rotation_file, ref_rot_longitude,
        ...
        for i in range(0, len(xopt_)):
            ...
            if ref_rotation_start_age<=80:
                results_dict.append({'Lon': xopt_lon, 'Lat': xopt_[i][0][1], 'Ang': xopt_[i][0][2], 'Minimum': xopt_[i][1],
                                     'Model': xopt_[i][3], 'Age': ref_rotation_start_age, 'NR':xopt_[i][2][0], 'NR_my':xopt_[i][2][1],
                                     'TM':xopt_[i][2][2],'TM_r':xopt_[i][2][3],'TM_a':xopt_[i][2][4], 'TM_mvo':xopt_[i][2][5], 
                                     'TM_mavo':xopt_[i][2][6],'TM_mvp':xopt_[i][2][7], 'HS':xopt_[i][2][8], 'HS_m':xopt_[i][2][9], 'HS_sd':xopt_[i][2][10],
                                     'PV':xopt_[i][2][11],'PV_m':xopt_[i][2][12]})
            else:
                results_dict.append({'Lon': xopt_lon, 'Lat': xopt_[i][0][1], 'Ang': xopt_[i][0][2], 'Minimum': xopt_[i][1],
                                     'Model': xopt_[i][3], 'Age': ref_rotation_start_age, 'NR':xopt_[i][2][0], 'NR_my':xopt_[i][2][1],
                                     'TM':xopt_[i][2][2],'TM_r':xopt_[i][2][3],'TM_a':xopt_[i][2][4], 'TM_mvo':xopt_[i][2][5], 
                                     'TM_mavo':xopt_[i][2][6],'TM_mvp':xopt_[i][2][7],
                                     'PV':xopt_[i][2][8],'PV_m':xopt_[i][2][9]})
        ...
        #results_dataframe.to_csv(r'C:/Users/6408885/optAPM-master/optAPM-master/model_output/CSV/'+str(model_name)+str(ref_rotation_start_age)+'.csv') 
       ...
       # stage_min_result = results_dataframe.head(1)
        ...
        # if plot_cost:
        #     #make an array with minimum cost
        #     cost = np.array(results_dataframe['Minimum'])
        ...
        if plot:
            ...
            pmap = plt.axes(projection=ccrs.Robinson())
            pmap.set_extent([-180, 180, -90, 90], ccrs.PlateCarree())
            #pmap.coastlines(linewidth=0.25)
            pmap.add_feature(cfeature.LAND, zorder=1, color='lightgray')
            pmap.gridlines(draw_labels=True)
            ...
            xall, yall = (np.array(results_dataframe['Lon']), np.array(results_dataframe['Lat']))
            xten, yten = (np.array(top_ten['Lon']), np.array(top_ten['Lat']))
            xmean, ymean = (np.float64(top_ten_mean['Lon']), np.float64(top_ten_mean['Lat']))
            xmin, ymin = (np.float64(min_result['Lon']), np.float64(min_result['Lat']))
            xref, yref = (ref_rot_longitude, ref_rot_latitude)
            slon, slat = (seed_lons, seed_lats)
            ...
            for i in range(0, len(results_dataframe)):
                ...
                if results_dataframe.iloc[i]['Ang'] < 0:
                    ...
                    pmap.scatter(xall[i], yall[i], marker='.', s=5, color='gray', zorder=2, transform=ccrs.PlateCarree())
                    ...
                    pmap.scatter(xall[i], yall[i], marker='o', s=abs(results_dataframe.iloc[i]['Ang']) * point_multiplier, c='gray',
                                 edgecolor='darkgray', zorder=2, label='All results' if i == 0 else "", transform=ccrs.PlateCarree())

                    pmap.scatter(xall[i], yall[i], marker='.', s=5, color='white', zorder=2, transform=ccrs.PlateCarree())
            ...
            for i in range(0, len(top_ten)):
                ...
                if top_ten.iloc[i]['Ang'] < 0:
                    ...
                    pmap.scatter(xten[i], yten[i], marker='o', s=abs(top_ten.iloc[i]['Ang']) * point_multiplier, c='green', zorder=2,
                                 label='10%' if i == 0 else "", edgecolor='seagreen', facecolor='none', linewidth=2, transform=ccrs.PlateCarree())

                    pmap.scatter(xten[i], yten[i], marker='.', s=5, c='green', zorder=2, transform=ccrs.PlateCarree())


                elif top_ten.iloc[i]['Ang'] > 0:

                    pmap.scatter(xten[i], yten[i], marker='o', s=abs(top_ten.iloc[i]['Ang']) * point_multiplier, c='green', zorder=2,
                                 label='10%' if i == 0 else "", transform=ccrs.PlateCarree())

                    pmap.scatter(xten[i], yten[i], marker='.', s=5, c='k', zorder=2, transform=ccrs.PlateCarree())


            # Plot top 10% mean
            if np.float64(top_ten_mean['Ang']) < 0:

                pmap.scatter(xmean, ymean, marker='o', s=abs(top_ten_mean['Ang']) * point_multiplier, c='orange', zorder=2,
                             label='10% mean', facecolor='none', edgecolor='orange', linewidth=2, transform=ccrs.PlateCarree())

                pmap.scatter(xmean, ymean, marker='.', s=5, c='orange', zorder=2, transform=ccrs.PlateCarree())

            elif np.float64(top_ten_mean['Minimum']) > 0:

                pmap.scatter(xmean, ymean, marker='o', s=abs(top_ten_mean['Ang']) * point_multiplier, c='orange', zorder=2,
                             label='10% mean', transform=ccrs.PlateCarree())

                pmap.scatter(xmean, ymean, marker='.', s=5, c='k', zorder=2, transform=ccrs.PlateCarree())


            # Plot min result
            if np.float64(min_result['Ang']) < 0:

                pmap.scatter(xmin, ymin, marker='o', s=abs(np.float64(min_result['Ang'])) * point_multiplier, c='blue', zorder=2,
                             label='Absolute minimum', facecolor='none', edgecolor='royalblue', linewidth=2, transform=ccrs.PlateCarree())

                pmap.scatter(xmin, ymin, marker='.', s=5, c='blue', zorder=2, transform=ccrs.PlateCarree())

            elif np.float64(min_result['Ang']) > 0:

                pmap.scatter(xmin, ymin, marker='o', s=abs(np.float64(min_result['Ang'])) * point_multiplier, c='blue', zorder=2,
                             label='Absolute minimum', transform=ccrs.PlateCarree())

                pmap.scatter(xmin, ymin, marker='.', s=5, c='k', zorder=2, transform=ccrs.PlateCarree())


            # Plot EarthByte ref result
            if np.float64(ref_rot_angle) < 0:

                pmap.scatter(xref, yref, marker='o', s=abs(ref_rot_angle) * point_multiplier, c='crimson', zorder=2,
                             label='Reference rotation', facecolor='none', edgecolor='crimson', linewidth=2, transform=ccrs.PlateCarree())

                pmap.scatter(xref, yref, marker='.', s=5, c='crimson', zorder=2, transform=ccrs.PlateCarree())

            elif np.float64(ref_result['Ang']) > 0:

                pmap.scatter(xref, yref, marker='o', s=abs(ref_rot_angle) * point_multiplier, c='crimson', zorder=2,
                             label='Reference rotation', transform=ccrs.PlateCarree())

                pmap.scatter(xref, yref, marker='.', s=5, c='k', zorder=2, transform=ccrs.PlateCarree())


            # Plot seed points
            pmap.scatter(slon, slat, marker='.', s=10, c='k', zorder=2, transform=ccrs.PlateCarree())
        ...
        return min_result, results_dataframe
...
class ObjectiveFunctions():
    ...
    def hotspot_trail_misfit(trail_data, ref_rotation_start_age, rotation_model, use_trail_age_uncertainty, trail_age_uncertainty):
        ...
        for i in range(0, len(trail_data)):
            ...
            #Changed interval from 10 to 5. Really strange they didn't do this
            
            if trail_data[i]['Age'][-1] >= ref_rotation_start_age - 10:
            #if trail_data[i]['Age'][-1] >= ref_rotation_start_age - 5:
                opt_results = []
                ...
                for j in range(0, len(trail_data[i]['Lon'])):
                    ...
                    #With hotspots now interpolated to the timesteps, this should work
                    #if trail_data[i]['Age'][j] == ref_rotation_start_age:
